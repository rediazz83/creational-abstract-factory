...
public class VehiculoElectricidadServiceImpl implements VehiculoService {
    ...
    private List<Automovil> crearAutomoviles(int cantidadAutos) {
        ...
        for(int index = 0; index < cantidadAutos; index++) {
            Automovil auto = fabrica.creaAutomovil("estandar", "amarillo", 6+index, 3.2, TipoVehiculoEnum.ELECTRICIDAD);
    ...
    private List<Scooter> crearScooters(int cantidadScooters) {
        ...
        for(int index = 0; index < cantidadScooters; index++) {
            Scooter scooter = fabrica.creaScooter("clasico", "rojo", 2+index, TipoVehiculoEnum.ELECTRICIDAD);
